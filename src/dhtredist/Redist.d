...
public class DhtRedist : CliApp
{
    ...
    private static class SubdivideStrategy : Strategy
    {
        ...
        ***********************************************************************/
        ...
        override public RangeAssignment[] assignNewRanges_ (NodeHashRange[] nodes)
        {
            ...
            foreach (ip, nodes_on_this_server; nodes_by_server)
            {
                ...
                {
                    assigned_ranges ~= this.subdivideIndividualNodes(nodes_on_this_server);
                }
                ...
                else
                {
                    Stdout.newline.yellow.formatln(
                        ...
                        .default_colour;
                    auto server_range = getServerHashRange(nodes_on_this_server, ip);
        ...
        ***********************************************************************/
        ...
        unittest
        {
            ...
            // 2 -> 4
            test!("==")(canUseSimpleSubdivide([non_empty, empty, non_empty, empty]),
                    true);

            // 4 -> 6
            test!("==")(canUseSimpleSubdivide([non_empty, non_empty, non_empty,
                    non_empty, empty, empty]), false);

            // 2 -> 5
            test!("==")(canUseSimpleSubdivide([non_empty, empty, non_empty,
                    empty, non_empty]), false);

            // 3 -> 6
            test!("==")(canUseSimpleSubdivide([non_empty, empty, non_empty,
                    empty, non_empty, empty]), true);

            // 2 -> 7
            test!("==")(canUseSimpleSubdivide([non_empty, non_empty, empty,
                    empty, empty, empty, empty, empty, empty]), false);

            // 2 -> 8
            test!("==")(canUseSimpleSubdivide([non_empty, non_empty, empty,
                    empty, empty, empty, empty, empty, empty, empty]), true);
        }
        ...
        ***********************************************************************/
        ...
        private RangeAssignment[] subdivideIndividualNodes (NodeHashRange[] nodes_on_this_server)
        in
        {
        ...
        body
        {
            ...
            assert(empties.length + nonempties.length == nodes_on_this_server
                    .length);
            ...
            foreach (nonempty; nonempties)
            {
                Stdout.formatln("\tSub-dividing existing node {}:{} "
                        ~ "= 0x{:X16}..0x{:X16}:", nonempty.node.Address,
                        nonempty.node.Port, nonempty.range.min, nonempty.range
                        .max);
                ...
                foreach (i, ref subdiv_range; subdiv_ranges)
                {
                    ...
                    else
                    {
                        ...
                        Stdout.formatln(
                                ...
                                subdiv_range.min,
                                subdiv_range.max, node.node.Address, node.node
                                .Port);
                    }
        ...
        ***********************************************************************/
        ...
        private static HashRange getServerHashRange (NodeHashRange[] nodes_on_server,
                cstring ip)
        in
        {
        ...
        body
        {
            ...
            {
                enforce(node.range.is_valid, cast(istring)("Node on " ~ ip ~ " has an invalid range"));
            ...
            enforce(!server_range.is_empty,
                    cast(istring)("No node on " ~ ip ~ " has a defined range"));
        ...
        unittest
        {
            ...
            testThrown!()(getServerHashRange(node_hash_ranges([empty, empty, empty]),
                    ""));

            // Single set range ok
            test!("==")(getServerHashRange(node_hash_ranges([HashRange(0, 1)]),
                    ""), HashRange(0, 1));

            // Single set range, multiple empty ranges ok
            test!("==")(getServerHashRange(node_hash_ranges([HashRange(0, 1),
                    empty, empty]), ""), HashRange(0, 1));

            // Multiple set ranges ok
            test!("==")(getServerHashRange(node_hash_ranges([HashRange(0, 1),
                    HashRange(2, 3), HashRange(4, 5)]), ""), HashRange(0, 5));
        }
    }
    ...
    ***************************************************************************/
    ...
    public this ()
    {
        ...
        const desc = "initiates a redistribution of dht data by changing the " "hash ranges of the nodes";
        OptionalSettings options;
        options.usage = "";
        options.help = `Tool to initiate a redistribution of data within a dht. The standard use
case, when adding new nodes to a dht, is as follows:
    ...
    ***************************************************************************/
    ...
    {
        args("src").aliased('S').params(1).conflicts("ranges").help("XML file describing dht -- should contain the address/port of " "all nodes to be affected, including those which are currently empty.");
        args("ranges").aliased('r').params(1).conflicts("src").help("Special mode to generate evenly distributed hash ranges for " "the specified number of nodes, sending the results to stdout. Does " "not contact the dht.");
        args("execute").aliased('x').help("Send calculated redistribution to the " "dht. This option must be set if you want to actually trigger a " "redistribution -- the default mode of the program is a dry run only.");
        args("strategy").aliased('s').params(1).restrict(["extend", "subdivide"])
            .defaults("extend").help("Sets the redistribution strategy. 'extend' " "mode should be used when adding new servers to the dht -- it " "completely redistributes the data among the total set of nodes. " "'subdivide' mode should be used when adding new nodes to existing " "dht servers -- the data is redistributed such that it is not sent " "between servers.");
    }
    ...
    ***************************************************************************/
    ...
    protected override int run (Arguments args)
    {
        ...
        if (args("src").assigned)
        {
            ...
            if (no_change)
            {
                Stdout.green.formatln("\nNo change in node hash ranges. " "Not sending Redistribute requests.")
                    .default_colour;
                return 0;
            }
        ...
        else if (args("ranges").assigned)
        {
            ...
            {
                Stdout.formatln("0x{:X16} 0x{:X16}", node_range.min, node_range
                        .max);
            }
    ...
    ***************************************************************************/
    ...
    private NodeHashRange[] getHashRanges (RedistDhtClient dht)
    {
        ...
        nodes.sort((NodeHashRange e1, NodeHashRange e2) {
    ...
    ***************************************************************************/
    ...
    body
    {
        ...
        double fractional = cast(double)(max % nodes) / cast(double) nodes;
    ...
    ***************************************************************************/
    ...
    unittest
    {
        void testRange (ulong min, ulong max)
        {
            ...
            {
                test(HashRange(min, max).isTessellatedBy(calculateHashRanges(num_nodes,
                        min, max)));
            }
        }
    ...
    ***************************************************************************/
    ...
    private RedistributeInfo[NodeItem] getPerNodeRedistribution (RangeAssignment[] new_node_ranges)
    out (redist_infos)
    {
    ...
    body
    {
        ...
        for (size_t i; i < new_node_ranges.length; i++)
        {
            ...
            for (size_t j; j < new_node_ranges.length; j++)
            {
                ...
                {
                    redist_infos[i_node].redist_nodes ~= NodeHashRange(j_node, j_range);
                }
            }
        }
    ...
    ***************************************************************************/
    ...
    private void redistribute (RedistDhtClient dht,
            RangeAssignment[] new_node_ranges, RedistributeInfo[NodeItem] redist_infos)
    in
    {
        assert(dht !is null);
    }
        ...
        void notifier (RedistDhtClient.RequestNotification info)
        {
            if (info.type == info.type.Finished)
            {
                if (info.succeeded)
                {
                    ...
                    auto pcnt_done = 100.0f * cast(float) done / cast(float) new_node_ranges
                        .length;
                    Stdout.green.formatln("{}:{} finished. {}% of nodes done.",
                            info.nodeitem.Address,
                            info.nodeitem.Port, pcnt_done).default_colour.flush;
                }
                else
                {
                    ...
                    dht.schedule(dht.redistribute(info.nodeitem.Address,
                            info.nodeitem.Port,
                            &get_nodes, &notifier).context(info.context), retry_ms);
                }
            }
        }

        auto action = this.execute ? "Sending" : "Calculated";
        Stdout.newline.green.formatln("{} Redistribute requests to nodes:", action)
            .default_colour;
        ...
        foreach (i, new_node_range; new_node_ranges)
        {
            with (new_node_range)
            {
                ...
                auto overlap = cast(double) node.range.overlapAmount(
                        *new_range) / cast(double)(node.range.max - node.range
                        .min);
                Stdout.formatln("\t{}:{} = 0x{:X16}..0x{:X16} -> 0x{:X16}..0x{:X16} ({}% overlap)",
                        node.node.Address, node.node.Port, node.range.min,
                        node.range.max, new_range.min, new_range.max, overlap * 100.0f);
                ...
                if (this.execute)
                {
                    dht.assign(dht.redistribute(node.node.Address,
                            node.node.Port, &get_nodes, &notifier).context(i));
                }
            }
        }
