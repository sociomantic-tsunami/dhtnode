    ...
    private class DhtPerformance : CliApp
    {
        ...
        private struct PerformanceInfo
        {
            ...
            ***********************************************************************/
            ...
            private bool isNodeSlow (double cmp_time)
            {
                ...
                bool result = node_median > cmp_time *
                              (1 + this.alert_percentage);
        ...
        ***************************************************************************/
        ...
        override public void setupArgs (IApplication app, Arguments args)
        {
            ...
            restrict(["exists", "get", "put"]).help(
                "command to test (exists / "
                "get / put)");
            ...
            .
            help(
                "simulate some processing time (delay) when handling the "
            ...
            args("per-node-mode").aliased('N').params(0,
                1).defaults("4").
        ...
        ***************************************************************************/
        ...
        override public istring validateArgs (IApplication app, Arguments args)
        {
            ...
            {
                return
                    "--request-rate can't exceed 1.000.000 requests per second";
            }
        ...
        ***************************************************************************/
        ...
        override protected int run (Arguments args)
        {
            ...
            } while (this.iterations == 0 || this.iteration_count <
                     this.iterations);
            return 0;
        }
        ...
        ***************************************************************************/
        ...
        private SchedulingDhtClient initClient (
            cstring xml, uint connections,
            bool strict_handshake = true, size_t request_queue_size =
                256 * 1024)
        {
            Stdout.formatln("Initialising client connections from {}", xml);

            auto client = this.createClient(xml, connections,
                request_queue_size);
        ...
        ***************************************************************************/
        ...
        {
            auto client =
                new SchedulingDhtClient(this.epoll, connections,
                    request_queue_size);
        ...
        private void assignRequest (hash_t key)
        {
            ...
            case "exists":
                this.dht.assign(this.dht.exists(
                        this.channel, key, &this.existsCallback,
                        &this.notifier).
        ...
        ***************************************************************************/
        ...
        private void display ( )
        {
            ...
            const percentages =
            [0.5, 0.66, 0.75, 0.8, 0.9, 0.95, 0.98, 0.99, 0.995, 0.999, 1];
            ...
            {
                Stdout.formatln(
                    "Iteration {} of infinite. Time distribution of {} {} requests:",
            ...
            else
            {
                Stdout.formatln(
                    ...
                    this.iteration_count + 1, this.iterations, this.count,
                    this.command);
            }
            ...
            Stdout.formatln("\n{} requests ({,3:1}%) failed",
                ...
                (cast(float) this.global_info.error_count /
                 cast(float) this.global_info.requests.length) * 100.0);

            if (this.show_timeouts)
            {
                auto timed_out = this.global_info.requests.greaterThanCount(
                    this.timeout);

                Stdout.formatln("{} requests ({,3:1}%) took longer than {}μs",
                    timed_out,
                    (cast(float) timed_out /
                     cast(float) this.global_info.requests.length) * 100.0,
                    this.timeout);
            }
        ...
        private void displayTables (in double[] percentages, NodeItem[] nodes)
        {
            ...
            foreach (i, percentage; percentages)
            {
                ...
                foreach (node; nodes)
                {
                    ...
                    bool highlight =
                        i == percentages.length - 1 && info.isNodeSlow(
                            global_median);
            ...
            foreach (node; nodes)
            {
                ...
                sformat(this.str_buf, "{} ({,3:1}%) failed",
                    info.error_count,
                    (cast(float) info.error_count /
                     cast(float) info.requests.length) * 100.0);
            ...
            if (this.show_timeouts)
            {
                ...
                foreach (node; nodes)
                {
                    ...
                    auto timed_out = info.requests.greaterThanCount(
                        this.timeout);
        ...
        ***************************************************************************/
        ...
        private void getCallback (DhtClient.RequestContext context,
            in cstring data)
        {
        }
        ...
        ***************************************************************************/
        ...
        private void existsCallback (DhtClient.RequestContext context,
            bool data)
        {
        }
        ...
        ***************************************************************************/
        ...
        private void delay ( )
        {
            ...
            {
                ulong requests_time =
                    cast(ulong) this.global_info.avg_times.average *
                    this.parallel * 1_000;
